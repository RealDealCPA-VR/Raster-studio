...
crate compiled into the `studio-desktop` binary (W15-B: one of them,
`openh264-sys2`, carries Cisco's OpenH264 C++ source and compiles it into the
same binary at build time; see its row below). There is no bundled runtime, no
...
| `ravif` | 0.13.0 | BSD-3-Clause | `raster`, through `image`'s `avif` feature — W10-F: File ▸ Export / Export As write AVIF (`raster::codec::formats::avif::encode`); `image` builds it without `asm` (no nasm). It brings `rav1e` 0.8.1, `v_frame`, `av1-grain` (BSD-2-Clause), `avif-serialize` (BSD-3-Clause), `imgref` (CC0-1.0 OR Apache-2.0), `loop9`, `av-scenechange`, `y4m`, `maybe-rayon`, `arg_enum_proc_macro`, `simd_helpers`, `noop_proc_macro`, `new_debug_unreachable`, `nom` 8, `equator`, `aligned-vec` (MIT), `bitstream-io`, `itertools` 0.14, `num-bigint` / `num-integer` / `num-rational` / `num-derive`, `pastey`, `profiling-procmacros`, `aligned`, `as-slice`, `no_std_io2` (MIT OR Apache-2.0). W15-A: AVIF is *decoded* by `rusty_av1d` (below), in the decode worker process only. |
| `openh264` | 0.9.8 | BSD-2-Clause | `raster` — W15-B: MP4 export writes **H.264** by default (`raster::codec::formats::mp4::h264`, High profile). Rust bindings with default features off and only `source` on. It brings `openh264-sys2` 0.9.8 (BSD-2-Clause), which vendors **Cisco's OpenH264 C++ source** (BSD-2-Clause, Copyright (c) 2013 Cisco Systems; the licence text is `upstream/LICENSE` in that crate) and compiles it with the `cc` crate at build time — Cisco's prebuilt binary and its runtime download (the `libloading` feature) are not used; `wide` 1.7.1 and `safe_arch` 1.2.0 (Zlib OR Apache-2.0 OR MIT). Build-only: `cc`, `nasm-rs` (MIT OR Apache-2.0; x86 assembly only when `nasm` is on PATH, otherwise the C++ kernels), `walkdir` (Unlicense OR MIT). The OpenH264 *decoder* is compiled in as well (the crate builds both halves); W16-M: it decodes an MP4's H.264 track for a video layer (`raster::codec::formats::mp4::video`), and the application runs it only inside the decode worker process (`app_shell::dialogs::decode_worker`, kinds `video` / `video-window`). Patents: H.264 is a patented format; Cisco's royalty coverage applies only to its own binary, so a distributor shipping this source build should take its own view on AVC patent licensing. |
| `rav1e` | 0.8.1 | BSD-2-Clause | `raster` — W13-L: Export As writes MP4 video (`ExportFormat::Mp4`, `raster::codec::formats::mp4`): the AV1 encoder, named directly with default features off (no `asm` / nasm, no `threading` / rayon). The same version and dependencies `ravif` already brings (above), so nothing new enters `Cargo.lock`. The MP4 container is written by the project's own code; there is no muxer crate. W16-M: AV1 video tracks are decoded by `rusty_av1d` (below), H.264 ones by OpenH264 (above), in the decode worker only. |
...
| `rusty_av1d` | 1.2.0 | BSD-2-Clause | `raster` — W15-A: File ▸ Open reads AVIF (`raster::codec::formats::heif`), **only inside the decode worker process** (`app_shell::dialogs::decode_worker`), because it can panic on a damaged file; W16-M: also an MP4's AV1 track for a video layer (`raster::codec::formats::mp4::video`), in the same worker. A rav1d (Rust port of dav1d) fork with a Rust API; `default-features = false` with `bitdepth_8` and `bitdepth_16` (no `asm`, so no nasm and no C toolchain); its `rust-version` is 1.79. Its `src/rust_api.rs` carries dav1d-rs's MIT notice. It brings `av-data` 0.4.4 (MIT), `byte-slice-cast` (MIT), `strum` / `strum_macros` 0.27 (MIT), `atomig` / `atomig-macro` and `assert_matches` (MIT OR Apache-2.0), `heck` 0.5 (MIT OR Apache-2.0), `zerocopy` / `zerocopy-derive` 0.7 (BSD-2-Clause OR Apache-2.0 OR MIT), `byteorder` (Unlicense OR MIT), `to_method` 1.1 (CC0-1.0, below), and `parking_lot`, `paste`, `raw-cpuid`, `static_assertions`, `bitflags`, `cfg-if`, `bytes` and the `num-*` crates already in the tree. Not yet counted in the full-graph table below. |
| `heic-rs` | 0.1.1 | MIT OR Apache-2.0 | `raster` — W15-A: File ▸ Open reads HEIC / HEIF (HEVC) and parses the AVIF container (`raster::codec::formats::heif`), **only inside the decode worker process**, because its HEVC decoder can panic on a damaged file. Pure Rust, `#![forbid(unsafe_code)]`; `default-features = false` with `std` (no `parallel`, so no rayon pool); no dependencies of its own; its `rust-version` is 1.85. The `bench` feature (its synthetic HEVC builder) is a `raster` dev-dependency only. Not yet counted in the full-graph table below. |
| `hayro-jpeg2000` | 0.3.1 | Apache-2.0 OR MIT | `raster` — W16-L: File ▸ Open reads JPEG 2000 (`.jp2` / `.j2k` / `.jpf`, `raster::codec::formats::more_formats_w16::jp2`). Pure Rust, `#![forbid(unsafe_code)]`; `default-features = false` with `std` only (no `simd` / `fearless_simd`, no `image` / `moxcms` integration), so it brings no other crate. Pinned to 0.3.1: 0.4 declares `rust-version` 1.92, over the MSRV; 0.3.1 declares none and builds with 1.89. Not yet counted in the full-graph table below. |
| `wuff` | 0.2.9 | MIT | `text-engine` — W16-L: File ▸ Open loads `.woff` / `.woff2` fonts (`text_engine::webfont`), unwrapped to sfnt before they reach `fontdb`. Pure Rust; `default-features = false` with `brotli` and `z`, which use `brotli-decompressor` 5.0.3 (BSD-3-Clause / MIT), `flate2` and `bytes` (MIT), all already in the tree; its `rust-version` is 1.85. Not yet counted in the full-graph table below. |
...
- **OpenH264 (C++, inside `openh264-sys2`) — BSD-2-Clause.** W15-B: the one C++
  source in the binary (blake3's SIMD assembly is the other non-Rust code). Attribution and no-endorsement: Cisco's
  copyright line and licence text belong in the shipped licence file with the
  other BSD notices. The licence covers the code only; H.264 patent licensing
  is separate (see the `openh264` row).
- **`to_method` 1.1.0 — CC0-1.0.** A public-domain dedication (with a
  permissive fallback licence): no obligation at all. W15-A: it arrives
  through `rusty_av1d`.
