...
LUT, a `.atn` action set, a `.jsx` script, a video file, clipboard image data and an asset
...
| A hostile text-warp descriptor (W15-C) | The `TySh` warp descriptor is read by its own bounded reader (`psd::text::warp_spec`, needed because a Custom mesh is an `ObAr` of `UnFl` lists the generic reader does not model): depth is checked against `max_descriptor_depth`, every count against `max_descriptor_items` **and** the bytes remaining before anything is reserved, every key and name against `max_name_units`; an unknown value type, an unknown `warpStyle`, a non-`UnFl` array item or a cut anywhere yields no warp (the layer imports unwarped), and a mesh that is not exactly 16 finite points is dropped (the flat mesh stands in), never a panic (`text::warp_tests::a_truncated_warp_is_none_never_a_panic`). | `psd/src/text.rs` — `warp_spec`, `wd` |
...
| Colour modes read as RGB producing silently wrong pixels | W16-B: every Photoshop colour mode is decoded by its own path (`psd::colour_modes::to_working_rgb`), not read as RGB: an Indexed palette must be exactly 768 bytes or the file is refused, a Duotone ink record is parsed through checked reads and one that does not parse is reported (the document opens as its greyscale base) rather than guessed, and every plane is length-checked against the geometry before it is converted; only an unassigned mode code is refused by name. Since W10-F a `.psb` (version 2) is read through the same bounded cursor with 64-bit lengths (`PsdHeader::read_any`); any other version is refused with `UnsupportedVersion`. | `psd/src/header.rs`, `read.rs`, `colour_modes.rs`; `app-shell/src/psd_colour_modes.rs` |
| A linked smart object naming a path on the user's disk (W16-J) | **Not bounded by the file.** A `liFE` entry in `lnkE` names a path, and the import reads that file whole with `std::fs::read` (no size cap, no restriction on where the path points) and decodes it as the smart object's source under `ImportLimits`; a path that cannot be read or decoded opens the layer's cached pixels and names the reason. So a hostile `.psd` can make the editor read a file the user can read and, if it decodes as an image, show it in the document (where a later save or export would carry it). A `filterFX` smart-filter descriptor goes through the depth- and count-limited descriptor reader; a filter this build lacks opens the pixels instead. | `app-shell/src/psd_live.rs`; `psd/src/placed.rs`, `smart_filters.rs` |
...
| AVIF and HEIC (W15-A) | **Decoded only in a child process.** Both decoders are known to panic on damaged input (`rusty_av1d` 1.2.0's `unwrap()` on a missing reference frame header; `heic-rs` 0.1.1's slice index in `recon.rs`), and the release profile aborts on a panic, so the codec facade never decodes either in the editor: it hands the bytes to the decoder `app-shell`'s `decode_worker::install` registered, which runs the editor's own executable as `--decode-worker avif|heic` (stdin / stdout pipes, stderr discarded; the hidden entry runs before any console, logging, crash hook or window). The request carries `ImportLimits`; the child caps the file it reads at four times `max_alloc_bytes` and checks every declared size (`ispe`, grid output, the decoded frame, `irot`-turned size) against them before decoding. The parent checks the answer's header (width, height, depth, ICC length against `max_icc_bytes`) against `ImportLimits` **before** it reads or allocates a pixel, reads exactly the pixels the header implies, and refuses trailing bytes; a refusal message is at most 4 096 bytes. A worker still running 120 s after its spawn is killed; any exit but a clean 0, or a malformed answer, is reported as "the AVIF decoder crashed on this file" (or "the HEIC decoder ..." for a HEIC). Measured: of 1000 bit-flipped copies, 42 panicked in the worker and none reached the parent. With no worker installed (a library user, a test) AVIF and HEIC are refused by name. **Not covered:** the worker is not sandboxed (it runs with the editor's own rights and could, if a decoder were exploitable rather than merely panicking, read or write what the user can); memory inside the worker is bounded only by `ImportLimits` and the decoders' own checks, not by an OS limit; `rusty_av1d` is dav1d transpiled to Rust with `unsafe` code throughout, so a memory-safety bug in it would act inside the worker with the user's rights. | `raster/src/formats/heif.rs`, `formats/mod.rs`; `app-shell/src/decode_worker.rs`; `apps/studio-desktop/src/main.rs` |
...
| EPS PostScript (W15-E) | A PostScript **program** runs in-process in this build's own interpreter, so the bounds are on work, not only on bytes: at most `Budget::max_ops` (30 000 000) operations (each executed object, each element `bind` walks, each colour space parsed) and `Budget::time` (15 s) of wall clock, the clock read every 1 024 operations, so one operation's own work (copying or searching strings up to the memory budget, linear in their length since W15-E round 3: `search` was quadratic, 116 s for a 4 MiB string, `postscript::tests::search_is_linear_in_its_strings`) is not interrupted and can overrun the 15 s by that much; a memory budget (the smaller of 512 MiB and the import limits' allocation ceiling, at least 64 MiB) that every string, array, dictionary, decoded filter byte and image sample is charged against before it exists (nothing is freed during a run); operand stack `MAX_OPERAND_STACK` (100 000), dictionary stack 1 000, execution nesting `MAX_EXEC_DEPTH` (120, so PostScript recursion is an error, not a native stack overflow), `{ }` nesting 200 in the scanner, `MAX_FILTER_CHAIN` (32) filters over one file (each reads the one under it), `MAX_CSPACE_NESTING` (4) colour spaces in colour spaces (round 3: an `Indexed` space naming itself as its base recursed until `STATUS_STACK_OVERFLOW`; `postscript::tests::a_colour_space_that_names_itself_is_an_error_not_a_native_stack_overflow`), `gsave` 1 000, 2 000 000 points a path. Data nesting is not capped (a 40-byte loop builds an array chain a million deep inside every budget); instead arrays, dictionaries and files are freed by an iterative `Drop` over a work list (round 1 of W15-E freed arrays recursively: a `STATUS_STACK_OVERFLOW` that took the process down, pinned by `postscript::tests::deep_nesting_is_freed_without_a_native_stack_overflow`; round 2 still freed a file -> procedure -> array -> file chain recursively, since a filter over a procedure is outside the filter-chain bound, pinned by `postscript::tests::a_filter_chain_through_procedures_is_freed_without_a_native_stack_overflow`; both on a 1 MiB thread stack). `bind` walks with a work list, visits each array once and counts every element against the operation budget (round 2 recursed outside the budget: a procedure holding itself twice was a 2^64 walk that never ended; `postscript::tests::bind_over_a_procedure_that_holds_itself_twice_ends_quickly`). These are the nesting routes the tests cover, not a proof that no other route recurses. Every array and dictionary a run makes is tracked (a weak reference) and emptied when the run ends, so reference cycles (`systemdict` holds itself; a program may put an array into itself) are freed after each open instead of leaking up to the memory budget (`postscript::tests::a_finished_run_frees_every_array_and_dictionary_it_made_cycles_included`); the SVG it writes at most `MAX_SVG_BYTES` (96 MiB) and `MAX_SVG_ELEMENTS` (400 000), then drawn under `ImportLimits` by `resvg`; an image is checked against the import limits before its buffer exists. No file-system access: `file`, `run`, `deletefile`, `renamefile` raise errors and are reported. Integer arithmetic stays in 32 bits and every index is checked. A malformed file ending in an error is what the tests show for every truncation and 3 000 byte-flipped mutants of a sample (`postscript::tests::malformed_postscript_errors_and_never_panics`), not a proof: it runs in-process, so a panic or native stack overflow the tests missed would end the editor. | `raster/src/formats/postscript.rs` |
...
| A `.atn` action set (W13-E) | Read through the resource-file route (64 MiB cap, `MAX_RESOURCE_BYTES`); counts are checked against `MAX_ENTRIES` (16 384) before anything is reserved, strings against `MAX_ATN_STRING` (4 096), and descriptors go through the `psd` crate's depth- and count-limited reader. A step is only ever mapped onto this application's own menu and dialog routes (`atn::interpret`, and since W16-H `atn_more` for the adjustments, crop, transform, layer, selection, clipboard and filter steps); an unmapped step is listed as skipped and never executed. A Save As / Export step opens the export route's save picker (`Action::Export`), so an action cannot choose where a file is written. | `asset-store/src/resources/atn.rs`, `atn_more.rs`; `app-shell/src/atn_play.rs`, `atn_play_more.rs` |
| A video file (`.mp4` / `.m4v` / `.mov`, W16-M) | **Decoded only in the decode worker.** File ▸ Open, drops, Open Recent, the command line, File ▸ Place and the timeline's Add Media read the file through a 1 GiB cap (`MAX_VIDEO_FILE_BYTES`) and hand it to the editor's own executable run as `--decode-worker video` or `video-window:<first>:<count>`, the process the AVIF / HEIC decoders use (same pipes, 120 s deadline, any unclean exit reported as a crash). In the worker, `mp4::video` bounds the sample tables by `mp4::probe` (every sample inside the file, at most `MAX_PROBE_SAMPLES`), the frame count by `MAX_VIDEO_FRAMES` (1 000), the coded size by `ImportLimits` and the decoded RGBA bytes by `max_video_bytes` (1 GiB, or the allocation ceiling when lower) **before** a picture is decoded, and checks every decoded picture against the first one's size; the parent checks the answer's header against the same bounds before it reads a frame. The decoders are Cisco's OpenH264 (C++, compiled from source by `openh264-sys2`, not memory-safe) and `rusty_av1d` (panics on some damaged input); neither runs in the editor process. Codecs other than H.264 and 8-bit AV1, and Matroska / WebM / AVI containers, are refused by name. `studio-desktop`'s `tests/w16m_video_worker.rs` runs the real binary as the worker over 46 garbage, truncated and bit-flipped files (1 garbage, 5 truncations, 40 bit flips): the garbage and truncated ones are asserted to be errors, the bit-flipped ones come back as results (at least 5 refusals in all are required), and the test process survives every one and the worker still decodes a good file afterwards. **Not covered:** the worker is not sandboxed (it runs with the user's rights), as for AVIF / HEIC. W15-B: H.264 *export* runs the OpenH264 encoder in the editor process over its own composited frames, at a size checked (16x16 to 3840x2160, even) before it is called. | `raster/src/formats/mp4_video.rs`, `formats/mp4.rs`; `app-shell/src/decode_worker.rs`, `video_layers.rs` |
| JPEG 2000, Valve `.vtf`, FITS, DICOM (W16-L) | Read through the codec facade's cap of four times `ImportLimits::max_alloc_bytes` (`formats::read_bounded`); every reader checks the declared dimensions against `ImportLimits` before a pixel buffer exists, checks every offset before following it and bounds every loop over a declared count by the bytes that remain. JPEG 2000 is decoded in-process by `hayro-jpeg2000` 0.3.1 (`#![forbid(unsafe_code)]`, its `simd` feature off). FITS reads at most 1 000 header blocks and 64 HDUs; DICOM skips undefined-length sequences at most 16 deep and refuses JPEG-family, deflate and big-endian transfer syntaxes by name; a VTF's P8 format is refused by name. Each module's tests truncate and bit-flip a fixture through the whole decode; like every in-process reader, a panic the tests missed would end the editor. | `raster/src/formats/more_formats_w16.rs`, `w16_jp2.rs`, `w16_vtf.rs`, `w16_fits.rs`, `w16_dicom.rs` |
| An ASCII DXF (W16-L) | The same facade cap; at most `MAX_ENTITIES` (200 000) entities and `MAX_POINTS` (4 000 000) sampled points; the flat road draws its SVG through `svg_import::rasterize` (its size and allocation checks), and File ▸ Open maps the same geometry onto vector layers through the SVG layer reader. A binary DXF is refused by name. | `raster/src/formats/w16_dxf.rs`; `app-shell/src/import_layered_w16.rs` |
| Clip Studio `.clip`, zipped Pixelmator Pro `.pxd`, CorelDRAW `.cdr`, InDesign `.indd` (W16-L: previews only) | A `.clip`'s SQLite chunk is read by this build's own read-only SQLite table reader: table b-trees only, each page visited at most once per walk, tree depth capped at 40, every payload checked against a byte cap before it is allocated and every offset before it is followed. The zipped `.pxd` and CorelDRAW X4+ files go through a small ZIP reader of this build's own (the central directory walked once, never past the file; a deflated entry inflates through a reader capped at the caller's cap; ZIP64 and encrypted entries refused by name). An `.indd`'s thumbnail is base64 in its XMP packet; a RIFF `.cdr`'s `DISP` chunk is a header-less Windows bitmap. Every preview is then decoded by the codec facade under `ImportLimits`. Affinity and SAI files are refused by name without being parsed. | `raster/src/formats/w16_clip.rs`, `w16_sqlite.rs`, `w16_pxd.rs`, `w16_zip.rs`, `w16_foreign.rs` |
| A Krita `.kra`'s layers (W16-L) | The ZIP through the bounded reader of the `.kra` row above; `maindoc.xml` walked by a small tokenizer nested at most 64 deep; at most 4 096 layers; each tile's offset and length checked against the file and its LZF output capped at the tile's size; the layer count and the total pixels allocated for the layers checked against `ImportLimits` before each layer is allocated. | `raster/src/formats/kra_layers.rs`; `app-shell/src/import_layered_w16.rs` |
| SVG, EPS and one-page PDF / `.ai` files opened as layers (W16-I) | An SVG is parsed exactly as the flat importer parses it (the 64 MiB file and inflated-text caps, the `data:`-only image resolver, the panic guard) and its `usvg` tree walked into layers; an element that cannot be a live layer is drawn by `resvg` under `ImportLimits`. The EPS layers come from the bounded PostScript interpreter's display list (its row above). A PDF page's content stream is read by this build's own small interpreter over `hayro-syntax`'s operator iterator, writing at most `MAX_LIST_BYTES` (32 MiB) of display list per page; a page using images, form XObjects, shadings, clips, other colour spaces, Type0 / Type3 fonts or soft masks is refused with the reason and rendered by `hayro` instead. | `raster/src/svg_layers_w16.rs`, `formats/pdf_layers_w16.rs`, `formats/postscript.rs`; `app-shell/src/import_vector_w16.rs` |
| A Curves preset `.acv`, a `.3dl` or SpeedGrade `.look` LUT (W16-L) | Each file is read through a 16 MiB cap (`MAX_BYTES`) before it is parsed, and every table is validated by `adjustments::Lut3d::new` (edge 2..=65, finite entries). | `app-shell/src/resource_import_w16.rs` |
| A WOFF / WOFF2 font (W16-L) | Opened like any font file (row above: read whole, **no size cap** on the file); the header's declared sfnt size is checked against `MAX_SFNT_BYTES` (64 MiB) before `wuff` 0.2.9 (pure Rust) unwraps it, and `wuff` caps its own decompressed size and compression ratio. | `text-engine/src/webfont.rs` |
...
A `.psd`'s linked smart object is read from the path it names (W16-J, §3).
Layer ▸ Smart Object ▸ Embed Linked (W11-E) also reads
