...
| A video file (MP4, MOV, WebM, AVI) | Refused by name from its signature before anything is parsed (`formats::mp4::video_refusal`): no video decoder is reachable. W15-B: H.264 *export* links Cisco's OpenH264 C++ code (compiled from source by `openh264-sys2`, not memory-safe); `openh264-sys2` compiles its decoder half as well, but no application code calls it (only `formats::mp4`'s tests do), and the encoder only ever reads the editor's own composited frames, at a size checked (16x16 to 3840x2160, even) before it is called. `mp4::probe`, which reads this build's own MP4 exports back in tests, refuses a sample table naming more than `MAX_PROBE_SAMPLES` (1 000 000) frames or more than the file holds before sizing anything by it. | `raster/src/formats/mod.rs`, `formats/mp4.rs` |
