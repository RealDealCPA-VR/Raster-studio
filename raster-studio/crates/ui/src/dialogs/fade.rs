...
impl FadeSpec {
    ...
    /// [`Self::fade`] over two equal-length RGBA8 buffers.
    pub fn fade_rgba8(&self, before: &[u8], after: &[u8]) -> Vec<u8> {
        let unit = |p: &[u8]| -> [f32; 4] { std::array::from_fn(|c| f32::from(p[c]) / 255.0) };
    ...
    pub fn fade_rgba16(&self, before: &[u16], after: &[u16]) -> Vec<u16> {
        let unit = |p: &[u16]| -> [f32; 4] { std::array::from_fn(|c| f32::from(p[c]) / 65535.0) };
        let mut out = Vec::with_capacity(after.len());
        for (b, a) in before.chunks_exact(4).zip(after.chunks_exact(4)) {
            let p = self.fade(unit(b), unit(a));
