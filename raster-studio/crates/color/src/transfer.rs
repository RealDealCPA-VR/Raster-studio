...
mod tests {
    ...
    fn every_finite_input_produces_a_finite_output() {
        for &c in &[-1e5f32, -100.0, -1.0, -0.5, -0.056, -0.04, 0.0, 0.04, 1.0, 1e5] {
            assert!(srgb_to_linear(c).is_finite(), "srgb_to_linear({c}) not finite");
            assert!(linear_to_srgb(c).is_finite(), "linear_to_srgb({c}) not finite");
        }
    }
    ...
    fn channel_wise_helpers_apply_per_channel() {
        ...
        assert_eq!(
            srgb_to_linear3(rgb),
            [srgb_to_linear(0.0), srgb_to_linear(0.5), srgb_to_linear(1.0)]
        );
        assert_eq!(
            linear_to_srgb3(rgb),
            [linear_to_srgb(0.0), linear_to_srgb(0.5), linear_to_srgb(1.0)]
        );
        // Channel order must not be permuted.
        assert_ne!(srgb_to_linear3([0.1, 0.2, 0.3]), srgb_to_linear3([0.3, 0.2, 0.1]));
    }
}
